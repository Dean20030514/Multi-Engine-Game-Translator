...
def _run_retranslate_phase(
    ...
    _print(
        f"[RETRANSLATE] 补翻完成: {rt_total_translated} 条, API 用量: {rt_client.usage.summary()}"
    )
    ...
    if tl_report_path.exists():
        ...
        if tl_size > _MAX_REPORT_JSON_SIZE:
            report["stages"]["tl_mode"] = {
                "error": (
                    f"tl_mode_report.json 过大 ({tl_size} > {_MAX_REPORT_JSON_SIZE})，视为损坏跳过"
                )
            }
        else:
            try:
...
def _run_pilot_phase(
    ...
    try:
        ...
        if pilot_db_path.exists():
            # Round 52 C4 BREAKING: default_language= retired.
    ...
    try:
        ...
        if full_report_path.exists():
            ...
            if full_size > _MAX_REPORT_JSON_SIZE:
                raise ValueError(
                    f"full report.json 过大 ({full_size} > {_MAX_REPORT_JSON_SIZE})，视为损坏跳过"
                )
            # Round 49 Step 2: TOCTOU defense via check_fstat_size on the open fd.
            ...
            report["stages"]["full"]["checker_dropped"] = int(
                full_report.get("total_checker_dropped", 0)
            )
...
def _generate_default_language(output_game_dir: Path, target_lang: str = "zh") -> bool:
    """在输出 game/ 目录中生成 default_language.rpy，设置游戏默认语言。

    Round 52 C4 BREAKING: hardcoded to ``config.default_language = "chinese"``
    (matches the Ren'Py tl/chinese/ subdir convention).  ``target_lang``
    kwarg kept for signature stability but ignored.
