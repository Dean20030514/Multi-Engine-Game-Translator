...
class APIConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if is_reasoning_model(self.model) and self.timeout < 300:
            logger.info(
                f"[API] 推理模型 {self.model} 检测到，timeout 从 {self.timeout}s 提升到 300s"
            )
            self.timeout = 300.0
        self._resolved = True
...
class APIClient:
        ...
        for attempt in range(1, self.config.max_retries + 1):
            try:
                ...
                if status == 404:
                    ...
                    wait = base + jitter
                    logger.warning(
                        f"429 限速，等待 {wait:.1f}s 后重试 ({attempt}/{self.config.max_retries})"
                    )
                    time.sleep(wait)
                    continue
                elif status >= 500:
                    ...
                    wait = base + jitter
                    logger.warning(
                        f"{status} 服务端错误，等待 {wait:.1f}s 后重试 ({attempt}/{self.config.max_retries})"
                    )
                    time.sleep(wait)
                    continue
                else:
                ...
                if attempt < self.config.max_retries:
                    ...
                    wait = base + jitter
                    logger.warning(
                        f"网络错误: {e}, 等待 {wait:.1f}s 后重试 ({attempt}/{self.config.max_retries})"
                    )
                    time.sleep(wait)
                    continue
                raise
    ...
    def _call_custom(self, system_prompt: str, user_prompt: str) -> str:
        ...
        raise RuntimeError("自定义引擎模块必须实现 translate_batch() 或 translate() 函数")
