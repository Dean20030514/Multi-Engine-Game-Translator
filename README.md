...
- **最新版**：[v0.7.0](../../releases/latest)（`GameTranslator-v0.7.0.zip`）
- **校验完整性**：下载后与同名 `.sha256` 比对——PowerShell：`certutil -hashfile GameTranslator-v0.7.0.zip SHA256`；解压后单文件 `gt.exe` 的哈希见包内 `checksums.txt`。
...
完整条款见本库 [使用条款与免责声明](使用条款与免责声明.md)。

版权方如认为本工具相关内容涉嫌侵权，可通过 GitHub Issues 或爱发电私信（可私下提交材料）联系；我们将在核验权属后按适用规则处理。
