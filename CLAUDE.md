...
7. **方案先行** — 给出改动方案和受影响函数列表，等用户确认后再写代码（**Auto Mode 例外**：< 10 行 trivial fix / 纯文档 / memory 变更直接执行；触及 15 hard contracts（见下文"维护规则"段 #1-#15）/ 800 行 cap / VERIFIED-CLAIMS / file_safety / Mock target / Repo rename consistency / .cursorrules byte-identical 任一契约面的改动仍必须先 plan）
...
- **长函数 cluster**（r67 audit L1 architectural decision，r67 文档化）：`translators/direct.py:62 run_pipeline 588 行` / `translators/tl_mode.py:128 run_tl_pipeline 470 行` / `translators/_tl_parser_selftest.py:20 run_self_tests 441 行` / `translators/_direct_file.py:72 translate_file 335 行` / `main.py:213 main 270 行` / `engines/generic_pipeline.py:207 run_generic_pipeline 267 行`。全局 rule（`~/.claude/rules/common/coding-style.md`）建议 < 50 行，但项目第 8 原则"最小改动"冲突——拆 588 行函数改动量大且历史 bug 风险高。**故意保留**——这些函数是已 stabilize 的多年代码（r1-r66 反复改进过），整段重构 ROI 远低于风险。**新加 production 函数 > 100 行必须先 plan-first** 评估是否拆分（避免该 cluster 持续增长）
